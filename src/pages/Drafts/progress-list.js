...
import { options, statusClasses, progressList } from "common/data/progress"
...
const DraftsProgressList = props => {
  ...
  function getContractorName(id) {
    ...
    if (clientForm.length == 1) {
      return "(" + (clientForm[0].first_name + " " + clientForm[0].last_name) + ")";
    } else if (clientForm.length > 1) {
      let todayDate = new Date();
      let firstDate = new Date(clientForm[0].createdAt);
      let minVal = todayDate - firstDate;
      let contractorName = (clientForm[0].first_name + " " + clientForm[0].last_name);
      for (let i = 1; i < clientForm.length; i++) {
        ...
        if (minVal > (todayDate - dateTemp)) {
          ...
          contractorName = (clientForm[i].first_name + " " + clientForm[i].last_name);
        }
      }
                          ...
                          <tr key={i}>
                            <td>
                              <h5 className="text-truncate font-size-14 m-0">
                                <Link to={`/contractors-overview/${item.id}`} className="text-dark">
                                  {item.engagement_name + getContractorName(item.id)}
                                </Link>
                              </h5>
                            </td>
                            <td>
                              <div className="text-center">
                                {(companies.filter(element => element.id.toString() == item.client_company.toString()))[0]?.company_name}
                              </div>
                            </td>
                            <td>
                              <div className="text-center" style={{ textOverflow: "clip", overflow: "auto", whiteSpace: "nowrap", maxWidth: "680px" }}>
                                {item.reason_recruit}
                              </div>
                            </td>
                            <td>
