...
import { Row, Col, Modal, ModalHeader, ModalBody, Label, Input, UncontrolledAlert } from "reactstrap";
...
const DraftsUncontrolledBoard = props => {
  ...
  const handleValidSubmit = async (values) => {
    try {
      ...
      if (data.success == "success") {
        ...
        await axiosApi().put(`api/update-progress`, reqData)
          .then(response => {
  ...
  const onCardDragEndEvent = (_card, source, destination) => {
    // if (((source.fromColumnId == 1 && destination.toColumnId == 2) || (source.fromColumnId == 2 && destination.toColumnId == 1)) && userRole != "ir35pro") {
    ...
    //   setModalSurvey(true);
    // }
    // if (destination.toColumnId != 2 && userRole == "ir35pro") {
    //   const updatedBoard = moveCard(updateBoard, source, destination);
    //   setBoard(updatedBoard);
        ...
        <Col>
          <Board
            ...
            renderCard={(data, { dragging }) => {
              return <CardTaskBox data={data} dragging={dragging} companies={props.companies} clientForms={props.clientForms}>
              </CardTaskBox>
            }}
          >{updateBoard}</Board>
        </Col>
...
DraftsUncontrolledBoard.propTypes = {
  board: PropTypes.any,
  companies: PropTypes.any,
  clientForms: PropTypes.any
}

export default DraftsUncontrolledBoard
